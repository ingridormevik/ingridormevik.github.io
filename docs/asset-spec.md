...
## Quarantine — art can break the core rule too

The rule is usually applied to text. It applies to pictures with equal force: a painted
cannon is a claim about armament, and a captioned photograph is a claim about a
building and a date.

`tools/build-pack-v2.py` cuts the concept pack into `assets/locations/` and
`assets/folklore/`, and diverts failing panels to `assets/quarantine/`. Quarantined art
never enters `data/pack-v2.json`, so the page builder cannot reach it — the refusal is
structural, not a note in a file someone has to read. Two panels are quarantined:

- **Sandviksbatteriet landscape** — depicts a naval cannon on a stone emplacement. The
  armament at this site is unresolved research, so the gun is invented, and the stop's
  own visual requirements say *no invented cannons or dates*. The stop currently ships
  with no banner at all, which is the correct outcome.
- **`12_history_archive.png`** — fabricated archival photographs with hallucinated
  captions and dates, legible as "Neevengardrent 1591" and "Slojtbaroe 1178". These are
  not the real buildings and the dates are invented. Reusable only as paper and frame
  texture with every caption painted out; never as an archival image, never behind the
  history lens.

Cleared location art still carries the line *"Concept art — a painted interpretation,
not a photograph of the site"* on every page, generated by the builder.
